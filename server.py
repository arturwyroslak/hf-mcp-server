...
_HEALTH_BODY = _dumps({"status": "ok", "v": "6.1.0"}).encode()
...
def _get_whoami() -> dict:
    ...
    if _whoami_cache is None and HF_TOKEN:
        try:
            ...
            log.info(f"[whoami] cached as '{_whoami_cache['name']}' ({_whoami_cache['type']})")
        except Exception as e:
            log.error(f"[whoami] failed: {e}")
            return {"error": str(e)}
...
def _cinv(repo_id, rt):
    ...
    if keys: log.info(f"[cache] invalidated {len(keys)} entries for {tag}")

async def _fetch(repo_id, fn, rt, max_sz=500_000):
    k = _ck(repo_id, fn, rt)
    c = _cget(k)
    if c:
        log.info(f"[read] CACHE HIT {rt}/{repo_id}/{fn} ({c.get('size', '?')} bytes)")
        return {**c, "_cached": True}
    log.info(f"[read] fetching {rt}/{repo_id}/{fn}")
    t0 = time.monotonic()
    try:
        ...
        log.info(f"[read] OK {fn} — {len(raw)} bytes in {time.monotonic()-t0:.2f}s")
        return res
    except httpx.HTTPStatusError as e:
        log.error(f"[read] HTTP {e.response.status_code} — {rt}/{repo_id}/{fn}")
...
async def _run(fn, *a, **kw):
    try:
        ...
        log.error(f"[run] timeout after {HF_UPLOAD_TIMEOUT}s — {fn.__name__}")
        return {"error": f"timeout {HF_UPLOAD_TIMEOUT}s"}
    except Exception as e:
        log.error(f"[run] {fn.__name__} failed: {e}")
        return {"error": str(e), "type": type(e).__name__}

def _safe(fn, *a, **kw):
    try: return fn(*a, **kw)
    except Exception as e:
        log.error(f"[safe] {fn.__name__}: {e}")
        return {"error": str(e)}
...
def hf_whoami() -> dict:
    ...
    log.info("[tool] hf_whoami")
    result = _get_whoami()
    log.info(f"[tool] hf_whoami → {result}")
    return result
...
async def hf_list_files(repo_id: str, repo_type: str = "space") -> dict:
    ...
    log.info(f"[tool] hf_list_files repo={repo_id} type={repo_type}")
    try:
        ...
        log.info(f"[tool] hf_list_files → {len(files)} items in {repo_id}")
        return {"files": files}
    except Exception as e:
...
async def hf_read_file(repo_id: str, path: str, repo_type: str = "space") -> dict:
    ...
    log.info(f"[tool] hf_read_file repo={repo_id} path={path}")
    result = await _fetch(repo_id, path, repo_type)
    if "error" in result:
        log.error(f"[tool] hf_read_file failed: {result['error']}")
    return result
...
async def hf_write_file(
    ...
    _rguard()
    log.info(f"[tool] hf_write_file repo={repo_id} path={path} size={len(content)}")
    t0 = time.monotonic()
    ...
    if isinstance(r, dict):
        log.error(f"[tool] hf_write_file error: {r}")
        return r
    log.info(f"[tool] hf_write_file OK {path} in {time.monotonic()-t0:.2f}s")
...
async def hf_write_many(
    ...
    log.info(f"[tool] hf_write_many repo={repo_id} files={paths} msg='{commit_message}'")
    t0 = time.monotonic()
    ...
    _cinv(repo_id, repo_type)
    if isinstance(r, dict):
        log.error(f"[tool] hf_write_many error: {r}")
        return r
    log.info(f"[tool] hf_write_many OK {len(files)} files in {time.monotonic()-t0:.2f}s")
...
async def hf_delete_file(
    ...
    _rguard()
    log.info(f"[tool] hf_delete_file repo={repo_id} path={path}")
    ...
    _cinv(repo_id, repo_type)
    if isinstance(r, dict):
        log.error(f"[tool] hf_delete_file error: {r}")
        return r
...
async def hf_create_space(
    ...
    log.info(f"[tool] hf_create_space repo={repo_id} sdk={sdk} files={len(files)}")
    t0 = time.monotonic()
    ...
    if isinstance(cr, dict) and "error" in cr:
        log.error(f"[tool] hf_create_space create failed: {cr}")
        return cr
    ...
    if isinstance(r, dict) and "error" in r:
        log.error(f"[tool] hf_create_space upload error: {r}")
        return {"repo_id": repo_id, "created": True, "upload_error": r["error"]}
    log.info(f"[tool] hf_create_space OK {repo_id} — {len(files)} files in {time.monotonic()-t0:.2f}s")
    return {"repo_id": repo_id, "url": f"https://huggingface.co/spaces/{repo_id}",
...
def hf_create_repo(name: str, repo_type: str = "model", private: bool = False) -> dict:
    ...
    log.info(f"[tool] hf_create_repo repo={repo_id} type={repo_type}")
...
def hf_repo_info(repo_id: str, repo_type: str = "space") -> dict:
    ...
    log.info(f"[tool] hf_repo_info repo={repo_id}")
...
def hf_search(content_type: str, query: str = "", author: str = "", limit: int = 20) -> dict:
    ...
    log.info(f"[tool] hf_search type={content_type} query='{query}' author='{author}'")
    try:
...
def hf_system_info() -> dict:
    ...
    return {"v": "6.1.0", "ro": HF_READ_ONLY, "token": bool(HF_TOKEN),
...
class LoggingMiddleware:
    """Logs every MCP request + health endpoint."""
    __slots__ = ("app", "_lh")
...
if __name__ == "__main__":
    log.info(f"Starting hf-mcp v6.1.0 on {MCP_HOST}:{MCP_PORT} workers={WEB_CONCURRENCY}")
    log.info(f"Token: {'SET' if HF_TOKEN else 'MISSING'} | ReadOnly: {HF_READ_ONLY}")
    _get_whoami()  # warm up whoami cache on startup
