...
def hf_file_operations(
    ...
    if action == "write":
        read_only_guard()
        r = safe_run(api.upload_file, path_or_fileobj=content.encode(), path_in_repo=filename,
    ...
    if action == "edit":
        ...
        try:
            ...
            r = safe_run(api.upload_file, path_or_fileobj=updated.encode(), path_in_repo=filename,
    ...
    if action == "delete":
        read_only_guard()
        r = safe_run(api.delete_file, path_in_repo=filename, repo_id=repo_id, repo_type=repo_type,
    ...
    if action == "backup":
        ...
        try:
            ...
            r = safe_run(api.upload_file, path_or_fileobj=raw.encode(), path_in_repo=bak,
                         repo_id=repo_id, repo_type=repo_type, commit_message=f"Backup {filename}")
        ...
        try:
            ...
            for fname in matched:
                ...
                if pattern in original:
                    api.upload_file(path_or_fileobj=original.replace(pattern, replacement).encode(),
                                    path_in_repo=fname, repo_id=repo_id, repo_type=repo_type,
...
def hf_repo_file_manager(
    ...
    if action == "file_write":
        return hf_file_operations("write", repo_id, filename=filename, repo_type=repo_type,
                                   content=content, commit_message=commit_message)
    if action == "file_edit":
        return hf_file_operations("edit", repo_id, filename=filename, repo_type=repo_type,
                                   old_text=old_text, new_text=new_text, commit_message=commit_message)
    if action == "file_delete":
        return hf_file_operations("delete", repo_id, filename=filename, repo_type=repo_type,
